...
The $R_{1}$ rates from simulations were first calculated separately for each C--H bond in the system (using Eq.~\eqref{eq:R1} with $N_\mathrm{H}=1$), and averaged over time; the thus obtained $N_\mathrm{l}$ measurements were then taken to be independent, allowing the uncertainty to be estimated as standard error of the mean.
%
The total $R_1$ rate of a given carbon was obtained as a sum over the $R_1$ rates of its C--H bonds.
%
...
The left panels of Fig.~\ref{fig:teff_R1} compare the $\tau_\mathrm{e}$ obtained for fully hydrated POPC bilayers in experiments (black) and in six different MD force fields (color).
...
CHARMM36 and Slipids have the best overall performance---although the $\tau_\mathrm{e}$ in Slipids exhibit a qualitatively wrong, decreasing, trend from $\mathrm g_{3}$ to $\mathrm g_{1}$.
...
\begin{figure*}[!ht]
\centering
\includegraphics[width=\textwidth]{./figures/normalcond_2020_2.pdf}
...
Slipids does (almost) perfect job for the $\beta$ and $\alpha$ segments;
...
In Fig.~\ref{fig:teff_R1} there are also cases where a matching $R_1$ is accompanied by a larger-than-experimental $\tau_\mathrm{e}$. Such combination suggests that MD does well at the 1\,ns scale, but has too slow long-time dynamics.
%
...
%
This is seen to be the case in all four all-atom force fields for the $\gamma$ segment,
