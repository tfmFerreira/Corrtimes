...
hold vast potential to decipher, e.g., molecular mechanisms behind anesthetics~\cite{chau07,XXX}, the effect of cholesterol on membrane structure~\cite{XXX,ferreira13}, lipid droplet formation~\cite{Vanni2017}, and the functioning of membrane proteins~\cite{lindahl08} \todo{add more references?}.     
...
$\tau_\mathrm e$ is effectively sensitive to all the time scales relevant for the lipid internal dynamics,
...
In contrast to $\tau_\mathrm e$, $R_1$ is more sensitive to motions with correlation times closer to reciprocals, $(\omega_H-\omega_C)^{-1}$, $\omega_C^{-1}$ and $(\omega_H-\omega_C)^-1$, where $\omega_H$ and $\omega_C$ are  the $^{1}$H and $^{13}$C Larmor frequencies (which depend on the magnetic field strength used in experiments). Therefore,the changes in $R_1$ are not intuitively related to changes of motional time-scales, e.g. for a C--H bond with rotational diffusion described by a single correlation time, a decrease of $R_1$ indicates that the correlation time of motion is farther appart from to the reciprocals above but does not enable to quantify the time-cale variation or even if the corresponding process became faster or slower. In practice C--H bond rotational diffusion is characterised by a number of distinct modes of motion with distinct correlation times which difficults even further the analysis. Conventionaly, motional time-scales and motion amplitudes are determined by measuring the dependence of $R_1$ on temperature and magnetic field strength and by fitting models which assume some motional geometries (e.g. site jumps, woble in a cone and others) and correlation times. MD simulations are specially important in this context since they provide motional models with no {\it{a priori}} assumption of the type of motion.            
